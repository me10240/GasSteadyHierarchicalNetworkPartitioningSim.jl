...
@testset "test GasLib-135 ideal run" begin
    ...
    exact_sol = GasSteadyHierarchicalNetworkPartitioningSim._parse_json("data/GasLib-135/exact_sol_ideal.json")
    ...
    @test isapprox(norm(x_dof1 - x_dof2, Inf), 0, atol = 1e-5)
...
@testset "test GasLib-135 simple CNGA run" begin
    ...
    x_dof1, _ = run_partitioned_ss(filepath, ss, eos=eos_var, cond_number=false, show_trace_flag=false, iteration_limit=2000, method=:trust_region)
    exact_sol = GasSteadyHierarchicalNetworkPartitioningSim._parse_json("data/GasLib-135/exact_sol_simple_cnga.json")
    _check_correctness(ss.sol, exact_sol)
    partition = create_partition_with_cut_points(ss; max_nodes= 15, allow_slack_node_partitioning=true, write_to_file = false, filepath = file * "partition-expt.json")
    x_dof2, _ = run_partitioned_ss(filepath, ss, eos=eos_var, cond_number=false, show_trace_flag=false, iteration_limit=2000, method=:trust_region)
    @test isapprox(norm(x_dof1 - x_dof2, Inf), 0, atol = 5e-5)
end
...
@testset "test GasLib-135 full CNGA run" begin
    ...
    x_dof1, _ = run_partitioned_ss(filepath, ss, eos=eos_var, cond_number=false, show_trace_flag=false, iteration_limit=2000, method=:trust_region)
    exact_sol = GasSteadyHierarchicalNetworkPartitioningSim._parse_json("data/GasLib-135/exact_sol_full_cnga.json")
    _check_correctness(ss.sol, exact_sol)
    partition = create_partition_with_cut_points(ss; max_nodes= 15, allow_slack_node_partitioning=true, write_to_file = false, filepath = file * "partition-expt.json")
    x_dof2, _ = run_partitioned_ss(filepath, ss, eos=eos_var, cond_number=false, show_trace_flag=false, iteration_limit=2000, method=:trust_region)
    @test isapprox(norm(x_dof1 - x_dof2, Inf), 0, atol = 5e-5)
