...
@testset "test 8 node ideal run" begin
    ...
    exact_sol = GasSteadyGeneralNetworkPartitioning._parse_json("data/8-node/exact_sol_ideal.json")
...
@testset "test 8 node simple CNGA run" begin
    ...
    @test solver_return.status == unique_physical_solution
    @test ref(ss, :node, 1, "withdrawal") * nominal_values(ss, :mass_flow) ≈ -275.00 atol = 1e-2
    exact_sol = GasSteadyGeneralNetworkPartitioning._parse_json("data/8-node/exact_sol_simple_cnga.json")
    _check_correctness(ss.sol, exact_sol)
end
...
@testset "test 8 node full CNGA run" begin
    ...
    @test solver_return.status == unique_physical_solution
    @test ref(ss, :node, 1, "withdrawal") * nominal_values(ss, :mass_flow) ≈ -275.00 atol = 1e-2
    exact_sol = GasSteadyGeneralNetworkPartitioning._parse_json("data/8-node/exact_sol_full_cnga.json")
    _check_correctness(ss.sol, exact_sol)
end
