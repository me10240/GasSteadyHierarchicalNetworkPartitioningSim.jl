...
@testset "test GasLib-24 ideal run" begin
    ...
    exact_sol = GasSteadySim._parse_json("data/GasLib-24/exact_sol_ideal.json")
...
@testset "test GasLib-24 simple CNGA run" begin
    ...
    _, _ = run_partitioned_ss(filepath, ss, eos=eos_var, cond_number=false, show_trace_flag=false, iteration_limit=2000, method=:trust_region)
    exact_sol = GasSteadySim._parse_json("data/GasLib-24/exact_sol_simple_cnga.json")
    _check_correctness(ss.sol, exact_sol)
end
...
@testset "test GasLib-24 full CNGA run" begin
    ...
    _, _ = run_partitioned_ss(filepath, ss, eos=eos_var, cond_number=false, show_trace_flag=false, iteration_limit=2000, method=:trust_region)
    exact_sol = GasSteadySim._parse_json("data/GasLib-24/exact_sol_full_cnga.json")
    _check_correctness(ss.sol, exact_sol)
end
