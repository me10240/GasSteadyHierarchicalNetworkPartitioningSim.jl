...
# how to do this cleanly without causing confusion
# need to store the given q in bc data struc always, since we need it for update in iteration
...
# so at each common node, find number of networks it belongs to, say n. need n-1 q vals to updaten   
...
using GasSteadySim
using JSON
file = "./data/8-node-split/"
file1 = "./data/8-node-split/N1/"
file2 = "./data/8-node-split/N2/"
...
ss1.ref[:node][4]["transfer"] = Dict{String, Any}("n1"=>0.36)
ss1.ref[:node][7]["transfer"] = Dict{String, Any}("n1"=>2.39) 
...
# wlog can we assume q is zero ? how does this affect update
#Assume BC for N1
# ss1.ref[:node][4]["withdrawal"] = 0.36 + ss1.boundary_conditions[:node][4]["val"]

# ss1.ref[:node][7]["withdrawal"] = 2.39 + ss1.boundary_conditions[:node][7]["val"]


function get_interface_dofs(ss1)
        return [ss1.ref[:node][4]["transfer"]["n1"], ss1.ref[:node][7]["transfer"]["n1"]]
end
...
for i = 1: 50
        ...
        # println(ss1.ref[:node][4]["potential"], "\t", ss1.ref[:node][7]["potential"]) 
        ...
        ss2.ref[:node][4]["potential"] = ss1.ref[:node][4]["potential"]  
        ss2.ref[:node][7]["potential"] = ss1.ref[:node][7]["potential"]
        ...
        #Update BC for N1
        ss1.ref[:node][4]["transfer"]["n1"] = 0.5 * ss1.ref[:node][4]["transfer"]["n1"] -0.5 * ss2.ref[:node][4]["withdrawal"]  
        ss1.ref[:node][7]["transfer"]["n1"] = 0.5 * ss1.ref[:node][7]["transfer"]["n1"] -0.5 * ss2.ref[:node][7]["withdrawal"]
