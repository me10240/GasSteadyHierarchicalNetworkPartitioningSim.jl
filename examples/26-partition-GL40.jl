...
# Alternate split:
# N2 = [9, 12, 17, 18, 22, 23, 25, 32, 33, 34, 35, 36, 37] U [21, 27]
# N1 = [1-40]\N1 U [21, 27]
# 21, 27 both have one transfer 




file = "./data/GasLib-40-split/"
file1 = "./data/GasLib-40-split/Ni/"
file2 = "./data/GasLib-40-split/Nii/"
file3 = "./data/GasLib-40-split/Niii/"
...
ss1.ref[:node][26]["transfer"] = Dict{String, Any}("n1"=>0, "n2"=>0)



ss2 = initialize_simulator(file2, eos=eos_var, initial_guess_filename="") 
println("n2 done")
ss3 = initialize_simulator(file3, eos=eos_var, initial_guess_filename="") 
println("n3 done")

df1 = prepare_for_nonlin_solve!(ss1)
df2 = prepare_for_nonlin_solve!(ss2)
df3 = prepare_for_nonlin_solve!(ss3)
...
function get_interface_dofs(ss1)
        # shld make it part of ref that updates after every solve
        return [ss1.ref[:node][26]["transfer"]["n1"], ss1.ref[:node][26]["transfer"]["n2"]]
end
...
function _combine_solutions!(ss::SteadySimulator, ss1::SteadySimulator, ss2::SteadySimulator, ss3::SteadySimulator)
    ...
    for i = 1:length(ref(ss3, :dof))
        comp, id = ss3.ref[:dof][i]
        if comp == :node
                x_dofs[ss.ref[comp][id]["dof"]] = ss3.ref[comp][id]["potential"]
        elseif comp == :pipe
                x_dofs[ss.ref[comp][id]["dof"]] = ss3.ref[comp][id]["flow"]
        elseif comp == :compressor
                x_dofs[ss.ref[comp][id]["dof"]] = ss3.ref[comp][id]["flow"]
        end
    end
...
for i = 1: 50
        ...
        println(ss1.ref[:node][26]["potential"])
        ...
        # BC for N2, N3 comes from N1 solution
        ss2.ref[:node][26]["potential"] = ss1.ref[:node][26]["potential"]
        ss3.ref[:node][26]["potential"] = ss1.ref[:node][26]["potential"]
        ...
        solver3 = solve_on_network!(ss3, df3)
        println(i, "c")

        println(solver3.status)

        #Update BC for N1
        ss1.ref[:node][26]["transfer"]["n1"] = 0.5 * ss1.ref[:node][26]["transfer"]["n1"] -0.5 * ss2.ref[:node][26]["withdrawal"]
        ss1.ref[:node][26]["transfer"]["n2"] = 0.5 * ss1.ref[:node][26]["transfer"]["n2"] -0.5 * ss3.ref[:node][26]["withdrawal"]
...
x_dof = _combine_solutions!(ss, ss1, ss2, ss3)
