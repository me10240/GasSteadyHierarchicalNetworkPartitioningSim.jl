...
file1 = "./data/GasLib-40-split/Na/"
file2 = "./data/GasLib-40-split/Nb/"
file = "./data/GasLib-40-split/"
...
ss1.ref[:node][21]["transfer"] = Dict{String, Any}("n1"=>0.0)
ss1.ref[:node][27]["transfer"] = Dict{String, Any}("n1"=>0.0) 
...
function get_interface_dofs(ss1)
        ...
        return [ss1.ref[:node][21]["transfer"]["n1"], ss1.ref[:node][27]["transfer"]["n1"]]
end
...
for i = 1: 50
        ...
        # for (i, _) in ss1.ref[:node]
        #                 println(ss1.ref[:node][i])
        # end
        # println(ss1.ref[:node][21]["potential"], "\t", ss1.ref[:node][27]["potential"]) 
        ...
        ss2.ref[:node][21]["potential"] = ss1.ref[:node][21]["potential"]  
        ss2.ref[:node][27]["potential"] = ss1.ref[:node][27]["potential"]

        solver2 = solve_on_network!(ss2, df2)
        println(i, "b")
        # for (i, _) in ss2.ref[:pipe]
        #                 println(ss2.ref[:pipe][i])
        # end
        ...
        #Update BC for N1
        ss1.ref[:node][21]["transfer"]["n1"] = 0.5 * ss1.ref[:node][21]["transfer"]["n1"] -0.5 * ss2.ref[:node][21]["withdrawal"]  
        ss1.ref[:node][27]["transfer"]["n1"] = 0.5 * ss1.ref[:node][27]["transfer"]["n1"] -0.5 * ss2.ref[:node][27]["withdrawal"]
...
# solver = solve_on_network!(ss, df, x_guess=x_dof, iteration_limit=10)
